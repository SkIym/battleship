...
export default class BoardRenderer {
  ...
  clueOnFirstHit(id) {
    ...
    const tilesToHighlight =
      this.game.currentPlayer.name === "player"
        ? dom.enemyTiles
        : dom.playerTiles;
    ...
    toHighlight.forEach((tile) => {
      ...
      if (a >= 0 && a <= 9 && b >= 0 && b <= 9) {
        const loc = a * 10 + b;
        tilesToHighlight[loc].style.backgroundColor =
          tilesToHighlight[loc].style.backgroundColor === "rgb(11, 51, 68)"
            ? "rgb(11, 51, 68)"
            : "rgb(112, 162, 184)";
  ...
  toggleTiles() {
    this.tiles =
      this.tiles === dom.enemyTiles ? dom.playerTiles : dom.enemyTiles;
  }
